...
function Remote(){
  ...
  source.addEventListener("watch:" + self.remoteId, function(event){
    getWatchers(event)
  })

  source.addEventListener("unwatch:" + self.remoteId, function(event){
    resetWatchers(event)
  })
  ...
  $(document).on('userplay', function(){
    self.status = 1
    self.start_at = player.currentTime()
    self.update()
  })
